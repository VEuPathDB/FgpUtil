...
public class ScriptTest {
  ...
  private final Case[] TEST_CASES = { new Case("true", true),
    new Case("false", false), new Case("true && false", false),
    ...
    new Case("'ryan' != ''", true), new Case("'ryan' == 'ry' + 'an'", true),
    new Case("'jam' + 'es' != 'james'", false), new Case("'' == ''", true) };
  ...
  private final Case[] JSON_CASES = { new Case("{ \"p1\": 3, \"p2\": 4 }", true),
    // properly escaped quotes
    new Case("{ 'p1': 3, 'p2': 'blah' }", false),
    // single quote strings disallowed
    new Case("{ p1: 3, p2: 2 }", false)
    // 'naked' field names disallowed
  };
  ...
  private final String BAD_PARAM_EXPR = "(params.p1 < 4";
  private final String GOOD_PARAM_EXPR = "(params.p1 == 3 && params.p2 < 4) || params.p1 > 5";

  private final Case[] PARAM_CASES = { new Case("{ \"p1\": 3, \"p2\": 2 }", true),
  ...
  public void booleanExprParseTest() {
    JavaScript evaluator = new JavaScript();
    assertTrue(evaluator.isValidBooleanExpression(GOOD_PARAM_EXPR));

  }

  @Test
  public void failedParamScriptParseTest() {
    JavaScript evaluator = new JavaScript();
  ...
  public void paramScriptTest() throws Exception {
    ...
    for (Case testCase : PARAM_CASES) {
      boolean result = evaluator.evaluateBooleanExpression(GOOD_PARAM_EXPR,
        testCase.expression);
      assertEquals(testCase.result, result);
    }
  }
  ...
  public void checkArrayTest() throws Exception {
    ...
    assertTrue(evaluator.evaluateBooleanExpression(containsCondition,
      "{ \"counts\": [ 4, 8, 3, 10 ] }"));
    assertFalse(evaluator.evaluateBooleanExpression(containsCondition,
      "{ \"counts\": [ 4, 8, 10 ] }"));
    assertFalse(evaluator.evaluateBooleanExpression(containsCondition,
      "{ \"counts\": [ ] }"));
  }
  ...
  @Test
  public void checkBadArrayTest() {
    JavaScript evaluator = new JavaScript();
    assertThrows(ScriptException.class, () ->
    evaluator.evaluateBooleanExpression("containsAny(params.counts, 5)",
      "{ \"counts\": [ 4, 8, 3, 10 ] }"));
  }
