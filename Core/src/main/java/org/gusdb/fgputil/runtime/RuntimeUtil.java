...
public class RuntimeUtil {
   ...
   * @param outputConsumer consumer which will be passed stderr output, and also stdout if stdoutFile is missing
   ...
   */
  public static Optional<Integer> executeSubprocess(
      List<String> command,
    ...
    Thread outputConsumptionThread = null;
    try {
      ...
      outputConsumptionThread = new Thread(() -> {
      ...
      if (exitWithoutTimeout) {
        waitForConsumptionThread(outputConsumptionThread);
      ...
      if (outputConsumptionThread != null && outputConsumptionThread.isAlive()) {
        outputConsumptionThread.interrupt();
      }
    }
  }

  private static void waitForConsumptionThread(Thread logMonitorThread) throws InterruptedException {
